...
(() => {
  ...
  const I18N = {
    fr: {
      'nav.refuge': 'Le refuge', 'nav.pieces': 'Les pièces', 'nav.elements': 'Les éléments', 'nav.where': 'Où', 'nav.book': 'Réserver',
      ...
      'ft.credit': 'Photos et vidéo par les propriétaires', 'ft.top': 'Haut de page', 'lang': 'EN', 'langAria': 'Switch to English'
    },
    en: {
      'nav.refuge': 'The cabin', 'nav.pieces': 'The rooms', 'nav.elements': 'The elements', 'nav.where': 'Where', 'nav.book': 'Book',
      ...
      'ft.credit': 'Photography and film by the owners', 'ft.top': 'Back to top', 'lang': 'FR', 'langAria': 'Passer au français'
    }
  };
  ...
  const src = (seq, i) => `frames/${seq}/f_${String(i).padStart(3, '0')}.webp`;
  chapters.forEach(ch => {
  ...
  function drawSeq(arr, t, scale, alpha) {
    ...
    drawImg(a, scale, alpha);
    if (frac > 0.02 && i + 1 < n && ready(arr[i + 1]) && arr[i + 1] !== a) drawImg(arr[i + 1], scale, alpha * frac);
  }
  ...
  [['refuge', '#refuge', '#pieces'], ['pieces', '#pieces', '#elements'], ['elements', '#elements', '#ou'], ['ou', '#ou', '.footer']].forEach(([id, start, endSel]) => {
